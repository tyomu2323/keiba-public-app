...
async function loadScoringRules(){const res=await fetch('/api/admin/scoring-rules',{headers:auth()});const j=await res.json();ruleList.innerHTML=(j.rules||[]).map(r=>`<div class="rec rule-row"><b>${r.enabled?'ON':'OFF'} ${r.name}</b><p>${r.category} / ${r.condition_json} / ${r.score}点</p><button onclick="toggleRule(${r.id})">ON/OFF</button><button onclick="deleteRule(${r.id})">削除</button></div>`).join('')||'ルールなし';}
