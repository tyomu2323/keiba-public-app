...
function renderRaceDetail(mode='final'){
 ...
 const entriesHtml=d.entries.map(e=>{const w=workoutMap[e.horse_id];const score=Number(e.score||0);const auto=Number(e.auto_score||0);const manual=Number(e.manual_score||0)+Number(e.rec_add_score||0);return `<tr><td class="mark-cell">${e.mark||''}</td><td>${horseNoBadge(e)}</td><td><b>${linkHorse(e)}</b><br><span class="muted">${e.sex_age||''}</span></td><td>${e.carried_weight?`${e.carried_weight}kg`:''}</td><td>${styleBadge(e.running_style)}</td><td>${linkJockey(e.jockey)}</td><td>${w?`<b>調教コース：${w.course}</b> ${w.intensity}<br><span class="muted">${w.lap_text} / ${pct(w.percentile)}</span>`:''}</td><td><span class="score-pill ${scoreClass(score)}">総合 ${score}</span><div class="score-split"><span>自動 ${auto}</span><span>手動 ${manual}</span></div>${renderScoreBreakdown(e,d.scores)}</td><td>${e.theoretical_odds||''}</td><td>${e.actual_odds||''}</td><td>${e.rec_reason||''}</td></tr>`}).join('');
 document.getElementById('detail').innerHTML=`<h3>${d.race.venue}${d.race.race_no}R ${d.race.name}</h3><p class="muted">${d.race.date} / ${d.race.surface}${d.race.distance}m / 馬場 ${d.race.going||'-'}</p>${d.bias?`<p><span class="pill">${d.race.venue} ${d.race.surface} バイアス</span>内${d.bias.inside} 外${d.bias.outside} 逃げ${d.bias.front} 先行${d.bias.stalker} 差し${d.bias.closer} 追込${d.bias.deep_closer||0}<br>${d.bias.comment}</p>`:''}<div class="table-wrap"><table class="newspaper"><thead><tr><th>印</th><th>枠/馬</th><th>馬</th><th>斤量</th><th>脚質</th><th>騎手</th><th>最終追切</th><th>加点</th><th>理論</th><th>実オッズ</th><th>理由</th></tr></thead><tbody>${entriesHtml}</tbody></table></div><div class="workout-switch"><h3>追い切り</h3><button onclick="renderRaceDetail('final')" class="${mode==='final'?'active':''}">最終追い切り</button><button onclick="renderRaceDetail('all')" class="${mode==='all'?'active':''}">全追い切り</button></div>${renderWorkoutTable(d.entries,d.workouts,mode)}`;
}
