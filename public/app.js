...
function courseRankingBlock(rows){
 if(!rows?.length)return '';
 const list=[...rows].sort((a,b)=>Number(a.rank||999)-Number(b.rank||999));
 const half=Math.ceil(list.length/2);
 const cols=[list.slice(0,half), list.slice(half)];
 const card=(r)=>`<div class="aptitude-rank-card rank-${r.rank<=3?'top':''}" title="${r.horse_name||''} / 同条件 ${r.same_condition} / 同距離 ${r.same_distance} / 同場所 ${r.same_venue}"><div class="aptitude-rank-left"><span class="aptitude-rank-order">${r.rank}</span>${coloredHorseNoOnly(r,'aptitude-horse-no')}</div><div class="aptitude-rank-main"><b>${r.horse_name||r.name||'-'}</b><div class="aptitude-breakdown aptitude-breakdown-inline"><span>同条件 <strong>${r.same_point}</strong></span><span>距離 <strong>${r.distance_point}</strong></span><span>場所 <strong>${r.venue_point}</strong></span></div></div><div class="aptitude-rank-point">${r.point}<small>点</small></div></div>`;
