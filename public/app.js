...
function setMyMark(raceId, horseId, mark){
 try{
  ...
  if(!mark) localStorage.removeItem(key); else localStorage.setItem(key, mark);
 }catch(e){}
...
function myMarkButtons(e){
 ...
 const label=cur || '-';
 return `<label class="my-mark-picker mark-${cur||'none'}" title="マイ印"><span class="my-mark-display">${label}</span><select class="my-mark-select" aria-label="マイ印" onchange="setMyMark('${raceId}','${e.horse_id}',this.value)">${USER_MARKS.map(m=>`<option value="${m}" ${cur===m?'selected':''}>${m||'-'}</option>`).join('')}</select></label>`;
}
...
function horseBarMini(rows, horseId=''){
 if(!rows?.length) return '<p class="muted">馬柱データなし</p>';
 return `<div class="horsebar-strip">${rows.map(p=>`<button class="horsebar-cell ${p.is_same_condition?'same-condition':''}" onclick="showPastRun('${horseId}','${encodeURIComponent(p.date||'')}')"><b>${p.finish_position||'-'}着</b><small>${p.venue}${p.surface}${p.distance}</small><small>${p.popularity||'-'}人気 ${p.odds||'-'}倍</small><small>差 ${p.margin||'-'} / 上${p.last_3f_rank||'-'}位</small></button>`).join('')}</div>`;
}
async function showPastRun(horseId,date){
 const j=await api('/api/horses/'+encodeURIComponent(horseId)+'/past-runs/'+encodeURIComponent(date));
 const p=j.past_run||{};
 detailModalBody.innerHTML=`<h2>過去レース詳細</h2><section><h3>${p.date||''} ${p.venue||''} ${p.race_name||''}</h3><p><b>${p.surface||''}${p.distance||''}m</b> / 馬場 ${p.going||'-'} / クラス ${p.class_name||'-'}</p><div class="table-wrap"><table><tbody><tr><th>着順</th><td>${p.finish_position||'-'}着</td></tr><tr><th>人気/オッズ</th><td>${p.popularity||'-'}人気 / ${p.odds||'-'}倍</td></tr><tr><th>着差</th><td>${p.margin||'-'}</td></tr><tr><th>騎手/斤量</th><td>${p.jockey||'-'} / ${p.carried_weight||'-'}kg</td></tr><tr><th>馬体重</th><td>${p.body_weight||'-'}kg ${p.body_weight_diff!==null&&p.body_weight_diff!==undefined?`(${Number(p.body_weight_diff)>0?'+':''}${p.body_weight_diff})`:''}</td></tr><tr><th>通過順</th><td>${p.passing_order||'-'}</td></tr><tr><th>上がり</th><td>${p.last_3f||'-'} / ${p.last_3f_rank||'-'}位</td></tr><tr><th>タイム</th><td>${p.time_text||'-'}</td></tr></tbody></table></div></section>`;
 openDetailModal();
}
...
function renderScoreBreakdown(e, scores){
 ...
 return `<details class="score-detail score-detail-compact"><summary>詳細</summary><div class="score-summary-mini"><span>自動 ${Number(e.auto_score||0)}</span><span>手動 ${Number(e.manual_score||0)+Number(e.rec_add_score||0)}</span><span>総合 ${Number(e.score||0)}</span></div><h4>自動加点</h4><ul>${autoRows.map(s=>`<li><b>${Number(s.score)>0?'+':''}${s.score}</b> ${s.rule_name}<br><span>${s.reason||''}</span></li>`).join('')||'<li class="muted">なし</li>'}</ul><h4>手動加点</h4><ul>${manualRows.map(s=>`<li><b>${Number(s.score)>0?'+':''}${s.score}</b> ${s.rule_name}<br><span>${s.reason||''}</span></li>`).join('')}${rec||''}${(!manualRows.length&&!rec)?'<li class="muted">なし</li>':''}</ul></details>`;
}

function horseBarRow(e){
 const rows=e.horse_bar||[];
 if(!rows.length) return '<span class="muted">-</span>';
 return `<div class="horsebar-mini">${rows.slice(0,5).map(p=>`<button class="bar-chip ${p.is_same_condition?'same-condition':''}" onclick="showPastRun('${e.horse_id}','${encodeURIComponent(p.date||'')}')"><b>${p.finish_position||'-'}着</b> ${p.venue}${p.surface}${p.distance}<br><small>${p.popularity||'-'}人 差${p.margin||'-'} 上${p.last_3f_rank||'-'}位</small></button>`).join('')}</div>`;
}
...
function workoutLegendInfo(){
 return `<div class="analysis-box workout-color-guide"><h3>追い切り色凡例</h3><div class="workout-legend compact-legend"><span class="legend easy15">馬なり 上位15%</span><span class="legend easy25">馬なり 上位25%</span><span class="legend hard15">強め/一杯 上位15%</span><span class="legend hard25">強め/一杯 上位25%</span><span class="legend best">🏆 自己ベスト</span></div></div>`;
}
function courseRankingBlock(rows, entries=[]){
 if(!rows?.length)return '';
 const list=[...rows].sort((a,b)=>Number(a.rank||999)-Number(b.rank||999));
 const half=Math.ceil(list.length/2);
 const cols=[list.slice(0,half), list.slice(half)];
 const entryMap={}; for(const e of (entries||[])){entryMap[String(e.horse_id)]=e; entryMap[String(e.horse_no)]=e;}
 const card=(r)=>{const e=entryMap[String(r.horse_id)]||entryMap[String(r.horse_no)]||{};const total=Number(e.score||0);const auto=Number(e.auto_score||0);const manual=Number(e.manual_score||0);return `<div class="aptitude-rank-card rank-${r.rank<=3?'top':''}" title="${r.horse_name||''} / 同条件 ${r.same_condition} / 同距離 ${r.same_distance} / 同場所 ${r.same_venue}"><div class="aptitude-rank-left"><span class="aptitude-rank-order">${r.rank}</span>${coloredHorseNoOnly(r,'aptitude-horse-no')}</div><div class="aptitude-rank-main"><b>${r.horse_name||r.name||'-'}</b><div class="aptitude-score-row"><span>総合 <strong>${total}</strong></span><span>自動 <strong>${auto}</strong></span><span>手動 <strong>${manual}</strong></span></div><div class="aptitude-breakdown aptitude-breakdown-inline"><span>同条件 <strong>${r.same_point}</strong></span><span>距離 <strong>${r.distance_point}</strong></span><span>場所 <strong>${r.venue_point}</strong></span></div></div><div class="aptitude-rank-point">${r.point}<small>点</small></div></div>`};
 return `<div class="analysis-box aptitude-box aptitude-box-wide"><div class="aptitude-head"><h3>コース適性ランキング</h3><span>${rows.length}頭</span></div><p class="muted">同条件：1着5点/2着4点/3着3点/4着1点、同距離：1着2点/2〜4着1点、同場所：1〜3着1点</p><div class="aptitude-ranking-columns">${cols.map(col=>`<div class="aptitude-ranking-col">${col.map(card).join('')}</div>`).join('')}</div></div>`
}


function adminMarksBlock(entries, courseRanking){
 const order={'◎':1,'○':2,'▲':3,'△':4,'☆':5,'消':6};
 const courseMap={};
 for(const r of (courseRanking||[])){courseMap[String(r.horse_id||r.horse_no)]=r;}
 ...
 return `<div class="analysis-box admin-recommend-box"><h3>推奨馬 <small>管理者印</small></h3><div class="admin-recommend-list">${rows.map(e=>{const apt=courseMap[String(e.horse_id)]||courseMap[String(e.horse_no)]||{};const aptText=apt.point!==undefined?` / 適性${apt.point}点`:'';return `<div class="admin-recommend-item mark-${e.mark}"><span class="admin-recommend-mark">${e.mark}</span>${coloredHorseNoOnly(e,'admin-recommend-no')}<div><b>${e.name||e.horse_name||'-'}</b><small>${e.running_style||'-'} / ${e.jockey||''} / 総合${Number(e.score||0)}${aptText}</small></div></div>`}).join('')}</div></div>`;
}

function scoreBox(e,score,auto,manual){return `<span class="score-pill ${scoreClass(score)}">総合 ${score}</span><span class="score-mini">自動 ${auto}</span><span class="score-mini">手動 ${manual}</span>`}
function setEntrySort(sortKey){currentEntrySort=sortKey;renderRaceDetail(currentWorkoutMode||'final');}
function sortedEntries(entries){const rows=[...(entries||[])];if(currentEntrySort==='score_desc') return rows.sort((a,b)=>Number(b.score||0)-Number(a.score||0)||Number(a.horse_no)-Number(b.horse_no));if(currentEntrySort==='score_asc') return rows.sort((a,b)=>Number(a.score||0)-Number(b.score||0)||Number(a.horse_no)-Number(b.horse_no));if(currentEntrySort==='popularity') return rows.sort((a,b)=>(Number(a.popularity)||999)-(Number(b.popularity)||999)||Number(a.horse_no)-Number(b.horse_no));return rows.sort((a,b)=>Number(a.horse_no)-Number(b.horse_no));}
function sortButton(key,label){return `<button onclick="setEntrySort('${key}')" class="${currentEntrySort===key?'active':''}">${label}</button>`}
function sortControls(){return `<div class="entry-sort-controls"><span>並べ替え</span>${sortButton('horse_no','馬番順')}${sortButton('score_desc','総合高い順')}${sortButton('score_asc','総合低い順')}${sortButton('popularity','人気順')}</div>`}
...
function renderRaceDetail(mode='final'){
 ...
 const entriesHtml=sortedEntries(d.entries).map(e=>{const w=workoutMap[e.horse_id];const score=Number(e.score||0);const auto=Number(e.auto_score||0);const manual=Number(e.manual_score||0);return `<tr><td class="my-mark-cell sticky-col sticky-mark">${myMarkButtons(e)}</td><td class="sticky-col sticky-frame">${horseNoBadge(e)}</td><td class="sticky-col sticky-horse"><b>${linkHorse(e)}</b> ${adminMarkSmall(e)}<br><span class="muted">${e.sex_age||''}</span></td><td>${e.carried_weight?`${e.carried_weight}kg`:''}</td><td>${styleBadge(e.running_style)}</td><td>${linkJockey(e.jockey)}</td><td>${e.popularity||'-'}人気<br><b>${e.actual_odds||'-'}倍</b></td><td>${bodyWeightText(e)}</td><td><div class="score-horizontal">${scoreBox(e,score,auto,manual)}${renderScoreBreakdown(e,d.scores)}</div></td><td>${horseBarRow(e)}</td><td>${conditionStatsMini(e)}</td><td>${w?`<div class="final-workout-cell"><span class="workout-course-mini">${w.course}</span><span class="workout-intensity-mini">${w.intensity}</span><span class="workout-lap-mini">${w.lap_text}</span><span class="workout-pct-mini">${pct(w.percentile)}</span></div>`:''}</td><td>${e.theoretical_odds||''}</td></tr>`}).join('');
 document.getElementById('detail').innerHTML=`<h3>${d.race.venue}${d.race.race_no}R ${d.race.name}</h3><p class="muted">${d.race.date} / ${d.race.surface}${d.race.distance}m / 馬場 ${d.race.going||'-'}</p>${d.bias?`<p><span class="pill">${d.race.venue} ${d.race.surface} バイアス</span>内${d.bias.inside} 外${d.bias.outside} 逃げ${d.bias.front} 先行${d.bias.stalker} 差し${d.bias.closer} 追込${d.bias.deep_closer||0}<br>${d.bias.comment}</p>`:''}<div class="race-analysis-grid race-analysis-top">${paceBlock(d.pace)}${adminMarksBlock(d.entries,d.course_ranking)}</div>${courseRankingBlock(d.course_ranking,d.entries)}${workoutLegendInfo()}${sortControls()}<div class="table-wrap"><table class="newspaper wide-table"><thead><tr><th class="sticky-col sticky-mark">マイ印</th><th class="sticky-col sticky-frame">枠/馬</th><th class="sticky-col sticky-horse">馬</th><th>斤量</th><th>脚質</th><th>騎手</th><th>人気/オッズ</th><th>馬体重</th><th>加点</th><th>馬柱</th><th>同条件成績</th><th>最終追切</th><th>理論</th></tr></thead><tbody>${entriesHtml}</tbody></table></div><div class="workout-switch"><h3>追い切り</h3><button onclick="renderRaceDetail('final')" class="${mode==='final'?'active':''}">最終追い切り</button><button onclick="renderRaceDetail('all')" class="${mode==='all'?'active':''}">全追い切り</button></div>${renderWorkoutTable(d.entries,d.workouts,mode)}`;
}
