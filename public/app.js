...
function courseRankingBlock(rows){
 ...
 return `<div class="analysis-box aptitude-box aptitude-box-wide"><div class="aptitude-head"><h3>コース適性ランキング</h3><span>${rows.length}頭</span></div><p class="muted">同条件：1着5点/2着4点/3着3点/4着1点、同距離：1着2点/2〜4着1点、同場所：1〜3着1点</p><div class="aptitude-ranking-list aptitude-ranking-list-vertical">${rows.map(r=>`<div class="aptitude-rank-card rank-${r.rank<=3?'top':''}" title="${r.horse_name||''} / 同条件 ${r.same_condition} / 同距離 ${r.same_distance} / 同場所 ${r.same_venue}"><div class="aptitude-rank-left"><span class="aptitude-rank-order">${r.rank}</span>${coloredHorseNoOnly(r,'aptitude-horse-no')}</div><div class="aptitude-rank-main"><b>${r.horse_name||r.name||'-'}</b><div class="aptitude-breakdown"><span>同条件 <strong>${r.same_point}</strong></span><span>距離 <strong>${r.distance_point}</strong></span><span>場所 <strong>${r.venue_point}</strong></span></div></div><div class="aptitude-rank-point">${r.point}<small>点</small></div></div>`).join('')}</div></div>`
}
